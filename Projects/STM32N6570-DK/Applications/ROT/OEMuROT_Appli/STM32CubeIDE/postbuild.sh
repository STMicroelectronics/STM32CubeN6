...
#=================================================================================================
bin_path_xml_field="../../../Applications/ROT/OEMuROT_Appli/Binary"
s_app_bin_xml_field=$bin_path_xml_field/rot_tz_s_app.bin
s_app_enc_sign_bin_xml_field=$bin_path_xml_field/rot_tz_s_app_enc_sign.bin
...
if  [ $signing == "secure" ]; then
  ...
  $python$applicfg xmlval -v "$s_app_bin_xml_field" --string -n "Firmware binary input file" "$s_app_init_xml" --vb >> "$current_log_file" 2>&1
  if [ $? != 0 ]; then error; fi
  ...
  $python$applicfg xmlval -v "$s_app_init_sign_bin_xml_field" --string -n "Image output file" "$s_app_init_xml" --vb >> "$current_log_file" 2>&1
  if [ $? != 0 ]; then error; fi

  # update xml file : output file
  $python$applicfg xmlval -v "$s_app_enc_sign_bin_xml_field" --string -n "Image output file" "$s_app_xml" --vb >> "$current_log_file" 2>&1
  if [ $? != 0 ]; then error; fi
