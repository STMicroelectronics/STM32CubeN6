...
#if USBD_AUDIO_PLAY_FREQ_MAX == USBD_AUDIO_FREQ_44_1_K
static UINT AUDIO_SpeakerInit(VOID);
static VOID AUDIO_SpeakerInitInjectionsParams(VOID);
#endif /* USBD_AUDIO_PLAY_FREQ_MAX == USBD_AUDIO_FREQ_44_1_K */
...
/**
  * @brief  Initializes the speaker audio process.
  * @param  None
  * @retval UX_SUCCESS on success, or UX_MEMORY_INSUFFICIENT if allocation fails.
  */
static UINT AUDIO_SpeakerInit(VOID)
{
  PlaybackAudioProcess.alt_buffer = ux_utility_memory_allocate(UX_SAFE_ALIGN, UX_CACHE_SAFE_MEMORY, SPEAKER_ALT_BUFFER_SIZE);

  /* Fail the initialization path if the alternate buffer cannot be allocated. */
  if (PlaybackAudioProcess.alt_buffer == UX_NULL)
  {
...
static VOID AUDIO_SpeakerInitInjectionsParams(VOID)
{
  /* Prepare the nominal and extended 44.1 kHz injection lengths. */
  ...
  PlaybackAudioProcess.injection_44_count = 0U;

  /* Clear the helper buffer before the wrap handler reuses it. */
...
/**
  * @brief  usbx_audio_play_app_thread
  *         This function handles the dedicated 44.1 kHz wrap processing path.
...
VOID usbx_audio_play_app_thread(ULONG arg)
{
  ...
  while (1)
  {
    /* Wait until the playback ring buffer has completed a full wrap. */
    ...
    case PLAY_BUFFER_OFFSET_FULL:
      /* Refresh the software read pointer once per full ring-buffer cycle so the 44.1 kHz helper stays aligned with the DMA consumer position. */
      BufferCtl.rd_ptr = USBD_AUDIO_PlaybackGetReadPosition();
      PlaybackAudioProcess.data_size = PlaybackAudioProcess.injection_size;

      /* Every tenth cycle, switch to the larger 44.1 kHz packet size and keep the nominal size on the other cycles. */
      if (PlaybackAudioProcess.injection_44_count < 9U)
      {
      ...
      diff = (uint16_t)(BufferCtl.size - BufferCtl.rd_ptr);
      if (diff < PlaybackAudioProcess.data_size)
      {
        /* Linearize the slice into the alternate buffer when the DMA reader is too close to the end of the ring buffer. */
        ux_utility_memory_copy(PlaybackAudioProcess.alt_buffer, &BufferCtl.buff[BufferCtl.rd_ptr], diff);
        ux_utility_memory_copy(PlaybackAudioProcess.alt_buffer + diff, &BufferCtl.buff[0], PlaybackAudioProcess.data_size - diff);
        PlaybackAudioProcess.data = PlaybackAudioProcess.alt_buffer;
      }
      break;
