...
#define AUDIO_SPEAKER_MAX_INJECTION_LENGTH(audio_desc) \
  (((audio_desc).audio_frequency == USBD_AUDIO_FREQ_44_1_K) ? \
   (AUDIO_SPEAKER_INJECTION_LENGTH(audio_desc) + \
    ((audio_desc).audio_channels_count * (audio_desc).audio_resolution)) : \
   AUDIO_SPEAKER_INJECTION_LENGTH(audio_desc))

#define AUDIO_SPEAKER_INJECTION_LENGTH(audio_desc)       (((audio_desc).audio_frequency / 8000U) * (audio_desc).audio_channels_count * 4U)
#define SPEAKER_ALT_BUFFER_SIZE                          (((USBD_AUDIO_PLAY_DEFAULT_FREQ + 7999U) / 8000U) * 2U * 3U * 2U)
#endif /* USBD_AUDIO_PLAY_FREQ_MAX == USBD_AUDIO_FREQ_44_1_K */
