...
#define MONITOR_INTERVAL_TICKS     1000
#endif
/* USER CODE END PD */
...
#if defined(TX_ENABLE_EXECUTION_CHANGE_NOTIFY) || defined(TX_EXECUTION_PROFILE_ENABLE)
float   Stream_thread_Execution= 0;
float   Idle_Execution= 0;
float   Isr_Execution= 0;
ULONG64 total= 0;
EXECUTION_TIME thread_time= 0, isr_time= 0, idle_time= 0, stream_thr_time= 0;
#ifdef UX_DEVICE_CLASS_AUDIO_FEEDBACK_SUPPORT
EXECUTION_TIME feedback_thr_time= 0;
float Feedback_thread_Execution;
#endif
#endif
/* USER CODE END PV */
...
VOID USBD_AUDIO_SetControlValues(VOID)
{
  ...
  audio_control[0].ux_device_class_audio20_control_sampling_frequency   = USBD_AUDIO_PLAY_DEFAULT_FREQ;
...
#if defined(TX_ENABLE_EXECUTION_CHANGE_NOTIFY) || defined(TX_EXECUTION_PROFILE_ENABLE)
/**
  * @brief  playback_monitor_thread
  *         Thread routine to monitor and log execution profile of system threads.
  *         Runs periodically to help track system performance.
  * @param  arg: Not used
  * @retval None
  */
void playback_monitor_thread(ULONG arg)
{
  while (1)
  {
    /* Retrieve the total execution time spent in system threads, ISRs, and idle */
    _tx_execution_thread_total_time_get(&thread_time);
    _tx_execution_isr_time_get(&isr_time);
    _tx_execution_idle_time_get(&idle_time);

    /* Calculate the overall total execution time */
    total= (ULONG64)thread_time + (ULONG64)isr_time + (ULONG64)idle_time;

    if (total)
    {
      USB_UsrLog("========= Thread Execution Profile =========");
      if (stream_read != NULL)
      {
        /* Get audio stream thread execution time and calculate usage percentage */
        _tx_execution_thread_time_get(&stream_read->ux_device_class_audio_stream_thread, &stream_thr_time);
        Stream_thread_Execution= ((float)stream_thr_time * 100) / total;
        USB_UsrLog("[TX_EXEC_PROFILE]: %s: %.3f%%", "ux_device_class_audio_stream_thread", Stream_thread_Execution);

#ifdef UX_DEVICE_CLASS_AUDIO_FEEDBACK_SUPPORT

        /* Get audio Feedback thread execution time and calculate usage percentage */
        _tx_execution_thread_time_get(&stream_read->ux_device_class_audio_stream_feedback_thread, &feedback_thr_time);
        Feedback_thread_Execution= ((float)feedback_thr_time * 100) / total;
        USB_UsrLog("[TX_EXEC_PROFILE]: Context: %s: %.3f%%", "ux_device_class_audio_feedback_thread",Feedback_thread_Execution);
#endif
      }

      /* Calculate and log idle time and ISR usage percentages */
      Idle_Execution = ((float)idle_time * 100) / total;
      Isr_Execution  = ((float)isr_time * 100) / total;
      USB_UsrLog("[TX_EXEC_PROFILE]: Context: %s: %.3f%%", "Idle System", Idle_Execution);
      USB_UsrLog("[TX_EXEC_PROFILE]: Context: %s: %.3f%%", "Interrupt", Isr_Execution);
    }
    tx_thread_sleep(MONITOR_INTERVAL_TICKS);
  }
}
#endif
