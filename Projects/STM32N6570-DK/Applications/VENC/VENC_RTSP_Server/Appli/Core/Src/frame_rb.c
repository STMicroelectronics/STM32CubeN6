...
void *  frb_alloc(uint32_t * nbBytes)
{
  ...
  /* If buffer is empty : restart from beginning in order to provide biggest chunck*/
  if  (iRead == iWrite)
  {
    hRb.iRead  = 0;
    hRb.iWrite = 0;
  ...
  /* Look for the largest  contiguous chunck*/
  ...
  /* returns maximum of contiguous bytes available*/
    *nbBytes = maxNbBytes;
