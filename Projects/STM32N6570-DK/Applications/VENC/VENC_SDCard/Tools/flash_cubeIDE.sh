...
SIGN_ONLY=0
if [ "$1" == "--sign-only" ] || [ "$1" == "--dry-run" ]; then
	SIGN_ONLY=1
	echo "Sign-only mode enabled: flashing steps will be skipped."
fi

# STM32CubeIDE output root path
export APth="../STM32CubeIDE"
...
APP_NAME="VENC_SDCard"

BIN_PATH="$APth/Appli/Debug/VENC_SDCard_Appli.bin"
FSBL_PATH="$APth/FSBL/Debug/VENC_SDCard_FSBL.bin"

if [ ! -f "$BIN_PATH" ]; then
	echo "Application binary not found: $BIN_PATH"
	echo "Build STM32CubeIDE application target first."
	exit 1
fi

if [ ! -f "$FSBL_PATH" ]; then
	echo "FSBL binary not found: $FSBL_PATH"
	echo "Build STM32CubeIDE FSBL target first."
	exit 1
fi
...
if [ "$SIGN_ONLY" == "1" ]; then
	echo "Sign-only mode: skipping bootloader/application flashing."
else
