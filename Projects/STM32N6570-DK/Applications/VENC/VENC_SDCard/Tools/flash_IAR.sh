...
BIN_PATH="$APth/Appli/VENC_SDCard_LRUN/Exe/Appli.bin"
FSBL_PATH="$APth/FSBL/VENC_SDCard_FSBL/Exe/FSBL.bin"

if [ ! -f "$BIN_PATH" ]; then
	echo "Application binary not found: $BIN_PATH"
	echo "Build IAR application target first."
	exit 1
fi

if [ ! -f "$FSBL_PATH" ]; then
	echo "FSBL binary not found: $FSBL_PATH"
	echo "Build IAR FSBL target first."
	exit 1
fi
