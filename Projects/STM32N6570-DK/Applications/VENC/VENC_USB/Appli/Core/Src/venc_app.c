...
#define H264_STREAM_START_MAX_SIZE 128U
#define VENC_APP_QUEUE_SIZE        15U
#define VENC_OUTPUT_BLOCK_NBR      4U
#define VENC_STARTUP_INTRA_FRAME_COUNT  3U
...
static uint32_t startup_intra_frames_remaining = 0U;
static uint8_t g_stream_start_blob[H264_STREAM_START_MAX_SIZE];
static uint32_t g_stream_start_blob_size = 0U;
...
/*
 * The encoder emits two different kinds of startup data:
 *
 * 1. H264EncStrmStart() returns a standalone byte-stream blob containing the
 *    sequence parameter set / picture parameter set headers needed by a fresh
 *    decoder.
 * 2. The first pictures returned by H264EncStrmEncode() are normal access
 *    units and may be IDR pictures, but they are not guaranteed to repeat the
 *    same SPS/PPS payload in the first encoded picture of the session.
 *
 * The current USB path therefore keeps the stream-start blob out of the normal
 * frame queue, caches it here, and lets USBX prepend it to the first host-
 * visible frame. This keeps the first USB-delivered access unit decodable
 * without inventing a synthetic "header frame" in the current queue contract.
 *
 * The blob cache is not a substitute for startup IDRs. It only provides the
 * codec headers. Decoder lock, transport resynchronization and host robustness
 * are improved further by sending several startup intra pictures after the blob
 * has been delivered.
 */
...
static int encoder_start(void)
{
  ...
  if ((ret == H264ENC_OK) && (encOut.streamSize != 0U))
  {
    ...
    if (encOut.streamSize <= H264_STREAM_START_MAX_SIZE)
    {
      (void)memcpy(g_stream_start_blob, (const void *)encIn.pOutBuf, encOut.streamSize);
      g_stream_start_blob_size = encOut.streamSize;
    }
    else
    {
      g_stream_start_blob_size = 0U;
      printf("[H264][strm_start] blob too large to cache: %lu\n", (unsigned long)encOut.streamSize);
    }
  }
 ...
 *
 * - Sends a short startup burst of intra pictures, then follows the regular GOP cadence.
...
static int encode_frame(void)
{
  ...
  /*
   * Startup uses a short IDR burst for a different reason than the blob cache.
   *
   * The cached H264EncStrmStart() payload gives the host the SPS/PPS headers
   * that the first encoded picture may not carry. The startup intra burst then
   * makes the opening phase more tolerant to packet loss, late host attachment,
   * and stream reopen timing by ensuring the first few queued pictures are all
   * independently decodable reference points.
   *
   * In other words:
   * - blob cache: guarantees parameter-set delivery at stream start
   * - startup IDRs: improves decoder lock and reopen robustness
   *
   * Both mechanisms are kept because they solve different startup problems.
   */
  if (startup_intra_frames_remaining != 0U)
  {
    /* Keep the first few pictures as IDR so the host can lock quickly after stream open. */
    encIn.codingType = H264ENC_INTRA_FRAME;
  }
  else if (!(frame_nb % hVencH264Instance.cfgH264Rate.gopLen))
  {
...
/* set input buffers to structures */  
  ...
  case H264ENC_FRAME_READY:
    ...
    if (startup_intra_frames_remaining != 0U)
    {
      startup_intra_frames_remaining--;
    }
    encIn.codingType = H264ENC_PREDICTED_FRAME;
     nb_encoded_frame++;
    break;
...
void VENC_APP_EncodingStart(void)
{
  ...
  nbLineEvent = 0U;
  nb_encoded_frame = 0U;
  startup_intra_frames_remaining = VENC_STARTUP_INTRA_FRAME_COUNT;
  g_stream_start_blob_size = 0U;
  timeMonitorStart();
...
{
  if ((data == NULL) || (size == NULL) || (g_stream_start_blob_size == 0U))
  {
    if (data != NULL)
    {
      *data = NULL;
  ...
  *data = g_stream_start_blob;
  *size = g_stream_start_blob_size;
  return 0;
}
