...
Message 6 received: TOPIC = Temperature, MESSAGE = 34
Message 7 received: TOPIC = Temperature, MESSAGE = 35
Message 8 received: TOPIC = Temperature, MESSAGE = 35
Message 9 received: TOPIC = Temperature, MESSAGE = 34
Message 10 received: TOPIC = Temperature, MESSAGE = 33
client disconnected from broker < broker.hivemq.com >.
 ```
...
4. To make an encrypted connection with the MQTT server, follow these steps to add an x509 certificate to the _mqtt_client_ for server authentication:
...
Below is an example of the section declaration for different IDEs.
   + For EWARM ".icf" file
   ```
   define symbol __ICFEDIT_region_NXDATA_start__ = 0x341EC000;
   ...
   + For MDK-ARM
   ```
   RW_NXDriverSection 0x341EC000 0x14000 {
   *(.NetXPoolSection)
   }
   ```
...
Next, this program can be run in boot from flash mode. This can be done by following the instructions below:
 ...
 - Resort to CubeProgrammer to add a header to the generated binary Project.bin with the following command
   - *STM32_SigningTool_CLI.exe -bin Nx_MQTT_Client.bin -nk -of 0x80000000 -t fsbl -o Nx_MQTT_Client-trusted.bin -hv 2.3 -dump Nx_MQTT_Client-trusted.bin*
       - The resulting binary is Nx_MQTT_Client-trusted.bin.
