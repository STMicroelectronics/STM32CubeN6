...
int main(void)
{
  ...
  /* Initialization of the LED_GREEN ------------------------- */
...
static void MX_XSPI1_Init(void)
{
  ...
  /* Update XSPIM configuration for XSPI1 */
  sXspiManagerCfg.IOPort = HAL_XSPIM_IOPORT_2;
...
static void MX_XSPI2_Init(void)
{
  ...
  /* Update XSPIM configuration for XSPI2 */
  sXspiManagerCfg.Req2AckTime = 10;
  if (HAL_XSPIM_Config(&hxspi2, &sXspiManagerCfg, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
  {
    Error_Handler();
...
void MPU_Config(void)
{
  ...
  */
  ...
  MPU_AttributesInit.Attributes = INNER_OUTER(MPU_WRITE_THROUGH|MPU_NON_TRANSIENT
                              |MPU_RW_ALLOCATE);
