...
static void MX_TIM1_Init(void)
{
  ...
  Prescaler = __LL_TIM_CALC_PSC(TimOutClock, 10000000);
  Period = __LL_TIM_CALC_PULSE(TimOutClock, Prescaler, 50, 50);
  Pulse = __LL_TIM_CALC_DELAY(TimOutClock, Prescaler, 50);
...
static void MX_TIM4_Init(void)
{
  ...
  */
  ...
  uint32_t Pulse;
  TimOutClock = GetTimerCLKFreq();
  Prescaler = __LL_TIM_CALC_PSC(TimOutClock, 40000);
  Period = __LL_TIM_CALC_PULSE(TimOutClock, Prescaler, 2000000, 3000000);
  Pulse = __LL_TIM_CALC_DELAY(TimOutClock, Prescaler, 2000000);
...
void TimerCaptureCompare_Callback(void)
{
  ...
  TimOutClock = GetTimerCLKFreq();
  uwMeasuredDelay = (cnt * 1000000)/(TimOutClock/(psc + 1));
  uwMeasuredPulseLength = ((arr - cnt) * 1000000)/(TimOutClock/(psc + 1));
}
