...
/**
  ...
  *          capture mode using the STM32N6xx TIM LL API.
...
int main(void)
{
  ...
  /* TIM4CLK = TIM_KER_CK / (APB prescaler & multiplier)                 */
  TimOutClock = GetTimerCLKFreq();
  /* TIM4 counter frequency = TimOutClock / (ARR + 1)                   */
...
__STATIC_INLINE void Configure_Frequency(uint32_t Frequency)
{
  ...
  /* Frequency = TIM4CLK / (ARR + 1)                   */
  LL_TIM_SetAutoReload(TIM4, __LL_TIM_CALC_ARR(TimOutClock, LL_TIM_GetPrescaler(TIM4), Frequency));
...
/**
  ...
  *         PWM signal generated by TIM4 is updated.
  * @param  None
  * @retval None
  */
