...
/**
  ...
  *          HSI as source clock through the STM32N6xx UTILS LL API.
  ******************************************************************************
  * @attention
  *
...
/* Configuration will allow to reach a SYSBUS frequency set to 320MHz:
   Syst freq = ((HSI_VALUE / PLLM) * PLLN)/ PLLP)
               ((64MHz     /  12  ) *   170)/  1  ) = 302MHz            */
LL_UTILS_PLLInitTypeDef sUTILS_PLLInitStruct = {12,170,1,1,0};

/* Variable to store AHB and APB buses clock configuration */
/* Settings to have SYSB set to 302MHz, APB1, APB2, APB4 and APB5 set to 151.1MHz */
LL_UTILS_ClkInitTypeDef sUTILS_ClkInitStruct = {LL_RCC_AHB_DIV_2,LL_RCC_APB1_DIV_1,LL_RCC_APB2_DIV_1,LL_RCC_APB4_DIV_1,LL_RCC_APB5_DIV_1};

/* Variable to store IC1, IC2, IC6, IC11 parameters */
LL_UTILS_ICInitTypeDef pUTILS_ClkInitStruct = {LL_RCC_ICCLKSOURCE_PLL1, 2,LL_RCC_ICCLKSOURCE_PLL1,3,LL_RCC_ICCLKSOURCE_PLL1,4,LL_RCC_ICCLKSOURCE_PLL1,3};
/* USER CODE END PV */
...
int main(void)
{
  ...
  LL_PLL_ConfigSystemClock_HSI(&sUTILS_PLLInitStruct, &pUTILS_ClkInitStruct, &sUTILS_ClkInitStruct);
  /*
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  LL_RCC_ConfigMCO(LL_RCC_MCO2SOURCE_SYSB, LL_RCC_MCO2_DIV_1);
}
