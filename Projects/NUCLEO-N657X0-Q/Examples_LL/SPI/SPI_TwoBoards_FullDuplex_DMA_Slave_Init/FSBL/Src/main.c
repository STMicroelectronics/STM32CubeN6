...
uint8_t aRxBuffer[sizeof(aTxBuffer)] __attribute__((section("noncacheable_buffer")));
...
static void MX_SPI5_Init(void)
{
  ...
  LL_RCC_SetSPIClockSource(LL_RCC_SPI5_CLKSOURCE_PCLK2);
  ...
  */
  ...
  DMA_InitStruct.Request = LL_GPDMA1_REQUEST_SPI5_RX;
  ...
  DMA_InitStruct.TriggerPolarity = LL_DMA_TRIG_POLARITY_MASKED;
  DMA_InitStruct.TriggerSelection = 0x00000000U;
  DMA_InitStruct.Request = LL_GPDMA1_REQUEST_SPI5_TX;
  DMA_InitStruct.TransferEventMode = LL_DMA_TCEM_BLK_TRANSFER;
  DMA_InitStruct.SrcAllocatedPort = LL_DMA_SRC_ALLOCATED_PORT0;
  DMA_InitStruct.DestAllocatedPort = LL_DMA_DEST_ALLOCATED_PORT0;
  DMA_InitStruct.LinkAllocatedPort = LL_DMA_LINK_ALLOCATED_PORT1;
  ...
  LL_DMA_ConfigAddresses(GPDMA1, LL_DMA_CHANNEL_2,
                         LL_SPI_DMA_GetRxRegAddr(SPI5), (uint32_t)aRxBuffer);
  ...
  LL_DMA_ConfigAddresses(GPDMA1, LL_DMA_CHANNEL_1,
                         (uint32_t)aTxBuffer, LL_SPI_DMA_GetTxRegAddr(SPI5));
...
//  LL_DMA_EnableChannel(GPDMA1, LL_DMA_CHANNEL_1);
  ...
  /* Configure SPI5 DMA transfer interrupts */
  /* Enable DMA RX Interrupt */
  LL_SPI_EnableDMAReq_RX(SPI5);
  /* Enable DMA TX Interrupt */
  LL_SPI_EnableDMAReq_TX(SPI5);
  /* Set transfer size */
  LL_SPI_SetTransferSize(SPI5, ubNbDataToTransmit);
...
/**
  * @brief  This function Activate SPI5
  * @param  None
  * @retval None
  */
void Activate_SPI(void)
...
/**
  ...
  * @retval None
  */
void SPI5_TransferError_Callback(void)
{
  /* Disable GPDMA1 Rx Channel */
  LL_DMA_DisableChannel(GPDMA1, LL_DMA_CHANNEL_2);
...
void MPU_Config(void)
{
  ...
  */
  LL_MPU_ConfigRegion(LL_MPU_REGION_NUMBER0, LL_MPU_REGION_PRIV_RW|LL_MPU_INSTRUCTION_ACCESS_ENABLE|LL_MPU_ACCESS_NOT_SHAREABLE, LL_MPU_ATTRIBUTES_NUMBER0, 0x34180000, 0x341FFFFF);
