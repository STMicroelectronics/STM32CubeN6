...
/* Definitions of data related to this example */
  ...
  #define SHADOW_REG_INDEX 110  /* hardware configuration register */
...
__IO uint32_t uhADCxConvertedData[ADC_CONVERTED_DATA_BUFFER_SIZE] __NON_CACHEABLE; /* ADC group regular conversion data (array of data) */
...
uint32_t uhADCxConvertedData_VrefInt_mVolt = 0UL;            /* Value of internal voltage reference VrefInt calculated from ADC conversion data (unit: mV) */
uint32_t uhADCxConvertedData_VrefAnalog_mVolt = 0UL;         /* Value of analog reference voltage (Vref+), connected to analog voltage supply Vdda, calculated from ADC conversion data (unit: mV) */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if(ubDmaTransferStatus == 1)
    {
      ...
      /* reload the fuse value */
      if(HAL_BSEC_OTP_Reload(&hbsec, SHADOW_REG_INDEX) != HAL_OK)
      {
        Error_Handler();
      }
      
      uhADCxConvertedData_VrefAnalog_mVolt = (((uint32_t)(*VREFINT_CAL_ADDR) * VREFINT_CAL_VREF) / __LL_ADC_CONVERT_DATA_RESOLUTION((uhADCxConvertedData[1]), (LL_ADC_RESOLUTION_12B), LL_ADC_RESOLUTION_12B));
