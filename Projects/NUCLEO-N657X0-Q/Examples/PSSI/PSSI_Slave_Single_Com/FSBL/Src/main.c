...
/**
  ******************************************************************************
  * @file    PSSI/PSSI_Slave_Single_Com/FSBL/Src/main.c
...
/* Buffer data received from Master */
...
int main(void)
{
  ...
  /* Wait for the Master to send a buffer with a timeout of 10s to give the time to the Master to send the data*/
  if (HAL_PSSI_Receive(&hpssi, aRxBuffer, RXBUFFERSIZE, 10000) != HAL_OK)
  {
    Error_Handler();
  }

  /* Compare buffer received with buffer sent */
  if (Buffercmp(aRxBuffer, aTxBuffer, RXBUFFERSIZE) != 0)
  {
    Error_Handler();
  }
