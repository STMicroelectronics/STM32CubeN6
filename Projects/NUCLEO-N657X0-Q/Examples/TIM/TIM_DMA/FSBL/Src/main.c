...
DMA_NodeTypeDef Node_GPDMA1_Channel7 __NON_CACHEABLE;
...
int main(void)
{
  ...
  In this example TIM1 input clock (TIM1CLK) is set to TIMPRE clock,
    since TIMPRE prescaler is equal to 1.
      TIMPRE = SYSB
      => TIM1CLK = HCLK = SystemCoreClock (600 MHz)

  TIM1CLK = 400000000, Prescaler = 0, TIM1 counter clock = 400000000
  ...
  uwTimerPeriod = (uint32_t)((400000000 / 17570) - 1);
