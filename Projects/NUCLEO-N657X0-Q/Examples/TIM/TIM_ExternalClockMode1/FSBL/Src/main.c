...
int main(void)
{
  ...
  while (1)
  {
    if (buttonPressedFlag != 0)
    {
      ...
      HAL_GPIO_WritePin(TIM_IN_GPIO_Port, TIM_IN_Pin, GPIO_PIN_SET);
      HAL_Delay(250);
      HAL_GPIO_WritePin(TIM_IN_GPIO_Port, TIM_IN_Pin, GPIO_PIN_RESET);
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_TIM_IN_CLOCK_ENABLE();

  /* Configure GPIO pin : TIM_IN_GPIO */
...
/* USER CODE BEGIN 4 */
/**
  * @brief  Update the button pressed flag when a rising edge is detected on the button pin.
