...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define VTOR_TABLE_NS_START_ADDR 0x241A0000
/* USER CODE END PD */
...
int main(void)
{
  ...
  __HAL_RCC_SPI1_CLK_ENABLE(); /* Peripheral clock need to be enabled */
  HAL_RIF_RISC_SetSlaveSecureAttributes(RIF_RISC_PERIPH_INDEX_SPI1, RIF_ATTRIBUTE_SEC | RIF_ATTRIBUTE_PRIV);

  /* Enable IAC interrupt */
  __HAL_RCC_IAC_CLK_ENABLE();
  HAL_RIF_IAC_EnableIT(RIF_RISC_PERIPH_INDEX_SPI1);
  ...
  static void SystemIsolation_Config(void)
{
  ...
  HAL_GPIO_ConfigPinAttributes(GPIOG,GPIO_PIN_8,GPIO_PIN_NSEC|GPIO_PIN_PRIV);
...
{
  /* Check if the correct interrupt has been triggered */
  if (PeriphId == RIF_RISC_PERIPH_INDEX_SPI1)
  {
    HAL_NVIC_DisableIRQ(IAC_IRQn);
