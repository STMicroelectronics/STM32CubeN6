...
int main(void)
{
  ...
  if (HAL_DTS_Start(&hdts, DTS_SENSOR_0) != HAL_OK)
  {
    /* DTS start Error */
    Error_Handler();
  }

  /* Wait for temperature available */
  if (HAL_DTS_PollForTemperature(&hdts, DTS_SENSOR_0, 1000U) != HAL_OK)
  {
  ...
  if (HAL_DTS_GetTemperature(&hdts, DTS_SENSOR_0, &Temperature) != HAL_OK)
  {
    /* DTS GetTemperature Error */
    Error_Handler();
  }
...
static void MX_DTS_Init(void)
{
  ...
  DTS_SensorConfigTypeDef pSensorParams = {0};
  ...
  pSensorParams.Mode = DTS_SENSOR_MODE_SINGLE;
  pSensorParams.Resolution = DTS_SENSOR_RESOLUTION_12BITS;
  if (HAL_DTS_ConfigSensor(&hdts, DTS_SENSOR_0, &pSensorParams) != HAL_OK)
  {
    Error_Handler();
  }
